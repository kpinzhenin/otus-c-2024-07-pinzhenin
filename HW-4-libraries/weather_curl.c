...
#include <curl/curl.h>
#include <cjson/cJSON.h>// data storage struct
...
void check_not_NULL_json(cJSON *json, char* error_msg);
		...
		curl_easy_setopt(curl, CURLOPT_URL, request);
			...
			if (json == NULL) {
				...
				{				
					...
					check_not_NULL_json(current_condittion_array, "Error. NULL pointer to item \"current_condition\"");	
					// get temperature in celsium
					cJSON *json_item_temp_c = cJSON_GetObjectItem(current_condittion_array, "temp_C");
					check_not_NULL_json(json_item_temp_c, "Error. NULL pointer to item \"temp_C\"");
					...
					check_not_NULL_json(current_condittion_array, "Error. NULL pointer to item \"windspeedKmph\"");	
				
					printf("discription: %s, temp: %s°C windspeed: %s kmph\n", json_weather_disc, json_temp_c_string, json_windSpeed_string);
				}
...
char* json_Get_request_string(cJSON *json)
{
	...
	check_not_NULL_json(json_request_array,"Error. NULL pointer to item \"request\"");

	// get it
	cJSON *json_query_city = cJSON_GetObjectItem(cJSON_GetArrayItem(json_request_array,0),"query");
	check_not_NULL_json(json_query_city, "Error. NULL pointer to item \"query\"");
